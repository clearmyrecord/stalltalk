-- Guarded issue scheduling migration for Neon/Postgres.
...
UPDATE "Issue" SET "isPublished" = true WHERE "status" = 'PUBLISHED' AND "isPublished" = false;
UPDATE "Issue" SET "isArchived" = true, "archivedAt" = COALESCE("archivedAt", "updatedAt") WHERE "status" = 'ARCHIVED' AND "isArchived" = false;
UPDATE "Issue" SET "isScheduled" = true, "scheduledPublishAt" = COALESCE("scheduledPublishAt", "scheduledAt") WHERE "status" = 'SCHEDULED' AND "isScheduled" = false;

CREATE UNIQUE INDEX IF NOT EXISTS "Issue_slug_key" ON "Issue"("slug") WHERE "slug" IS NOT NULL;
CREATE INDEX IF NOT EXISTS "Issue_schedule_publish_idx" ON "Issue"("isScheduled", "isPublished", "isArchived", "scheduledPublishAt");
CREATE INDEX IF NOT EXISTS "Issue_active_publish_idx" ON "Issue"("isPublished", "isArchived", "publishedAt");
