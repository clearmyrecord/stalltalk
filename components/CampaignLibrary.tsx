...
export function CampaignLibrary({ campaigns, issues }: { campaigns: Campaign[]; issues: Issue[] }) {
  async function action(campaign: Campaign, actionName: "duplicate" | "unpublish" | "archive" | "publish") {
    const issueId = actionName === "publish" ? window.prompt("Issue ID to publish into", campaign.issueId || issues[0]?.id || "") : "";
  ...
  return (
    <section className="grid gap-5">
      <div><p className="font-black uppercase tracking-[.25em] text-stallRed">AI Ad Studio</p><h1 className="font-display text-7xl uppercase">Campaign Library</h1><p className="font-bold">Draft, preview, version, publish, unpublish, archive, and download 3:1 sponsor banner content-ad campaigns.</p></div>
