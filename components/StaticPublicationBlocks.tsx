    ...
    <article className={`ad-card inline-ad ${primary ? "inline-ad-primary" : ""} ${image ? "" : "is-empty"}`} id={`ad-${slotNumber}`} data-ad-slot="content-ad" data-placement={slotNumber}>
      ...
      {image ? (
        ...
        <a className="ad-placeholder-link" href={href}>
          ...
          <span>Reach thousands of monthly readers from inside Las Vegas venues.</span>
          <small>Restaurants • Bars • Casinos • Attractions • Local Businesses</small>
...
export function StaticArticle({ title, body, variant }: { title: string; body: string; variant: "feature-card" | "secondary-card" }) {
  return (
    <section className={`publication-content-block panel ${variant}`}>
      <p className="feature-eyebrow">{variant === "secondary-card" ? "FEATURE STORY" : "FEATURE"}</p>
      <h2>{variant === "secondary-card" ? "LAS VEGAS: THE CITY THAT SHOULD NOT EXIST" : title}</h2>
      {variant === "secondary-card" ? <p className="feature-subtitle">How a desert railroad stop became one of the world's most visited cities.</p> : null}
...
function QuotesBlock() { return <section className="publication-content-block quotes-card panel"><h2>INSPIRATIONAL QUOTES</h2><div className="gold-accent" /><ul>{publishedIssue.quotes.map((quote) => <li key={quote}><span>{quote}</span><cite>Potty Favor</cite></li>)}</ul></section>; }
