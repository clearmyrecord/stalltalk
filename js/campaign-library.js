...
export async function generateResizeUploadAndSave({ generatedImageUrl, campaign }) {
  ...
  if (!slot) throw new Error("Select a valid ad placement before saving.");
  ...
  return saveCampaign({ ...campaign, width: slot.width, height: slot.height, image_url: cloudinary.secure_url });
}
...
export function createCampaignActionButtons(campaign, { onEdit, onReuse, onRefresh } = {}) {
  ...
  const actions = [
    ...
    ["Publish to Slot", () => publishCampaign(campaign.id, campaign.slot_id)],
    ["Download Image", () => downloadAdImage(campaign.image_url, campaign.slot_id, campaign.name)],
...
export async function mountCampaignLibrary(container, options = {}) {
  ...
  campaigns.forEach((campaign) => {
    ...
    card.innerHTML = `<h3>${campaign.name || "Untitled campaign"}</h3><p>${campaign.business_name || ""} · ${campaign.slot_id || "No slot"}</p>`;
