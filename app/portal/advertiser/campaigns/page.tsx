  ...
  const [campaigns, targets] = await Promise.all([
    ...
    prisma.venue.findMany({
      where: {
        ...
        status: "ACTIVE",
        issues: {
          some: { status: "PUBLISHED", isPublished: true, isArchived: false },
        },
      },
      select: {
        id: true,
        ...
        state: true,
        venueType: true,
        restrooms: { where: { status: "ACTIVE" }, select: restroomSelect },
        issues: {
    ...
    <main className="min-h-screen bg-paper p-8 text-ink">
      <section className="mx-auto max-w-5xl rounded-2xl border-4 border-ink bg-white p-6 shadow-brutal">
        ...
        <p className="mt-2 font-bold">
          Review submitted, draft, paid, and active advertiser campaigns.
        </p>
        <Link
        ...
        <div className="mt-6 grid gap-3">
          {campaigns.length ? (
            campaigns.map((campaign) => (
              ...
              >
                <p className="text-xs font-black uppercase text-stallRed">
                  {campaign.status} • Approval {campaign.approvalStatus} •{" "}
                  {money(campaign.totalAmountCents)}
                </p>
            ...
            <p className="rounded-xl border-2 border-ink bg-stallYellow p-4 font-black uppercase">
              No campaigns yet.
            </p>
          )}
        </div>
      ...
      <section className="mx-auto mt-6 max-w-5xl rounded-2xl border-4 border-ink bg-white p-6 shadow-brutal">
        <p className="font-black uppercase tracking-[.25em] text-stallPurple">
          Available public targeting
        </p>
        <h2 className="font-display text-5xl uppercase">
          Venues, Restrooms & Issues
        </h2>
        <p className="font-bold">
          Only active venues with published issues are listed here. Private
          venue drafts are not exposed.
        </p>
        <div className="mt-4 grid gap-3">
          {targets.map((venue) => (
            <article
              ...
              className="rounded-xl border-2 border-ink bg-paper p-4"
            >
              <h3 className="font-display text-3xl uppercase">{venue.name}</h3>
              <p className="font-black uppercase text-stallRed">
                {venue.city}, {venue.state} • {venue.venueType}
              </p>
              <div className="mt-2 grid gap-2 md:grid-cols-2">
                <label className="font-black uppercase">
                  Select restroom/type
                  <select
                    name="restroomTarget"
                    className="mt-1 w-full rounded border-2 border-ink p-2"
                  >
                    <option value={`venue:${venue.id}`}>Venue-wide</option>
                    {venue.restrooms.map((r) => (
                      <option key={r.id} value={`restroom:${r.id}`}>
                        {r.name} • {restroomTypeLabel(r)}
                      </option>
                    ))}
                  </select>
                </label>
                <label className="font-black uppercase">
                  Published issue/event
                  <select
                    name="issueTarget"
                    className="mt-1 w-full rounded border-2 border-ink p-2"
                  >
