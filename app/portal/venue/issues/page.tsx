...
export default async function VenueIssuesPage() {
  ...
  const venue = user.venueId ? await prisma.venue.findUnique({ where: { id: user.venueId }, include: { issues: { include: { venue: { select: { slug: true } }, restroom: { select: { slug: true, name: true } } }, orderBy: { updatedAt: "desc" } } } }) : null;
