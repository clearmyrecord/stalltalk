...
export default async function NewVenueIssuePage() {
  ...
  const venue = user.venueId ? await prisma.venue.findUnique({ where: { id: user.venueId }, select: { id: true, name: true, slug: true, publisherId: true, timeZone: true } }) : null;
  ...
  try {
    ...
    const draftIssue: VenueIssueDraft = {
      ...
      restroomId: null,
      qrCodeId: null,
      venue: { slug: venue.slug, timeZone: venue.timeZone },
      contentBlocks: [],
      adSlots: [],
    };
