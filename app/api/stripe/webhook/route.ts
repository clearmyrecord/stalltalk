...
export async function POST(request: Request) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (campaignId) {
      const campaign = await prisma.adCampaign.findUnique({ where: { id: campaignId }, include: { placements: { include: { inventory: true } }, inventory: true } });
      if (campaign) {
        const inventory = campaign.placements.length ? campaign.placements.map((placement) => placement.inventory) : campaign.inventory ? [campaign.inventory] : [];
        const conflicts = await findOverlappingCampaignPlacements(inventory, campaign.flightStartMonth, campaign.flightEndMonth, campaign.id);
        if (conflicts.length) {
          await prisma.adCampaign.update({ where: { id: campaignId }, data: { status: "REJECTED", approvalStatus: "REJECTED", rejectedAt: new Date(), rejectionReason: "A selected placement became unavailable for this flight before payment completed." } });
          await prisma.payment.updateMany({ where: { stripeSessionId: session.id }, data: { status: "FAILED", stripePaymentIntentId: typeof session.payment_intent === "string" ? session.payment_intent : null } });
        } else {
          const placementCount = Math.max(1, campaign.placementCount || inventory.length || campaign.locationCount);
          ...
          const paidCampaign = await prisma.adCampaign.update({ where: { id: campaignId }, data: { status: "PAID", paidAt: new Date(), stripeSessionId: session.id, placementCount, locationCount: placementCount, flightMonths, months: flightMonths, priceCents: totalAmountCents, totalAmountCents }, include: { inventory: true } });
          ...
          if (paidCampaign.approvalStatus === "APPROVED") await publishPaidCampaign(paidCampaign.id);
        }
      }
    }
  }
...
async function findOverlappingCampaignPlacements(inventory: Array<{ venueId: string; restroomId: string | null; qrCodeId: string | null; toiletLocationId: string | null; slotNumber: number }>, flightStartMonth: string, flightEndMonth: string, excludeCampaignId?: string) {
  ...
  return prisma.adCampaignPlacement.findMany({
    where: {
      ...
      campaign: { status: { in: ["PAID", "ACTIVE"] }, flightStartMonth: { lte: flightEndMonth }, flightEndMonth: { gte: flightStartMonth } }
    }
  });
}
