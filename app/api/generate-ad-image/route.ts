...
function currentModel(body: Record<string, unknown> = {}) {
  return safe(
    body.openAiImageModel || body.model || process.env.OPENAI_IMAGE_MODEL,
    "gpt-image-1",
  );
}
...
function mapOpenAiError(status: number, data: any) {
  ...
  if (
    ...
    (message.includes("billing") ||
      ...
      code.includes("hard_limit"))
  )
    return "billing_hard_limit";
  if (status === 429) return "rate_limit";
  if (
    message.includes("model") &&
    (message.includes("does not exist") || message.includes("invalid"))
  )
...
function buildCopy(body: Record<string, unknown>) {
  ...
  const audience = limitText(
    safe(body.audience || body.targetAudience, "nearby customers"),
    42,
  );
  ...
  try {
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Unable to parse OpenAI response JSON.";
    console.error("[generate-ad-image] OpenAI JSON parse error", {
      message,
...
async function saveGeneratedCreative(
  ...
  const versionNumber =
    requestedVersion > 0 ? requestedVersion : (latest?.versionNumber || 0) + 1;
  const campaignId = safe(
    body.campaignId,
  ...
  try {
    ...
    console.error("[generate-ad-image] Campaign history save failed", {
      error,
      message: rawMessage,
      campaignId,
      adSize,
    ...
    const message = missingColumn
      ...
      : "Campaign generated, but database history could not be saved. Check server logs and run the latest migration.";
    return {
      campaignId,
      parentCampaignId,
      versionNumber,
...
export async function POST(request: Request) {
  ...
  if (user && !["ADMIN", "ADVERTISER"].includes(user.role))
    return json(
      {
        error: "Forbidden",
        diagnostic: diagnostic({ errorType: "forbidden" }),
      },
      request,
      403,
    );
  ...
  try {
    ...
    return json(
      {
        error: message,
        diagnostic: diagnostic({ errorType: "json_parse_error" }),
      },
      request,
      400,
    );
  ...
  if (!process.env.OPENAI_API_KEY) {
    const message =
      "OPENAI_API_KEY is not configured on the server/Vercel environment.";
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const message =
        errorType === "billing_hard_limit"
          ? "OpenAI billing limit reached. Update billing in OpenAI Platform."
          : rawMessage;
      console.error("[generate-ad-image] OpenAI API error", {
        status: response.status,
        requestId,
        errorType,
        message,
    ...
    const imageBase64 = safe(
      image?.b64_json || image?.image_base64 || image?.imageBase64,
      "",
    );
    const imageUrl = imageBase64
      ? `data:image/${safe(process.env.OPENAI_IMAGE_OUTPUT_FORMAT, "png")};base64,${imageBase64}`
      : image?.url;
    if (!imageUrl) {
      const message =
        "OpenAI returned a successful response without image data.";
      console.error("[generate-ad-image] Missing image data", {
        requestId,
        model,
    ...
    const history = await saveGeneratedCreative(
      body,
      adSize,
      creative,
      imageUrl,
    ...
    return json(
      {
        ...
        metadata: {
          ...
          businessCategory: safe(
            body.businessCategory || body.category,
            "local business",
          ),
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Fetch to OpenAI image generation failed.";
    console.error("[generate-ad-image] Vercel/fetch function error", {
      message,
      model,
    });
    return json(
      {
        ...
        error: message,
        diagnostic: diagnostic({
          errorType: "fetch_or_function_error",
          error: message,
          model,
        }),
      },
