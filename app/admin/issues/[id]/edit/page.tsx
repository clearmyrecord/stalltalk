...
export default async function EditIssuePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!issue) notFound();
  return <section><div className="flex flex-wrap items-center justify-between gap-3"><h1 className="font-display text-7xl uppercase">Edit Issue</h1><Link className="rounded-xl bg-stallYellow px-4 py-3 font-black uppercase" href={`/issue/${issue.venue.slug}${issue.qrCode ? `?qr=${issue.qrCode.qrSlug}` : ""}`}>Preview Public Issue</Link></div><IssueForm publishers={publishers} venues={venues} restrooms={restrooms} qrCodes={qrCodes} articles={articles} ads={ads} issue={issue} /></section>;
}
