...
import { approveAdCampaign, approveVenueContentDraft, rejectAdCampaign, rejectVenueContentDraft } from "@/lib/actions";
...
const adminLinks = [
  ...
  ["Ad slot inventory", "/admin/venues"],
...
export default async function AdminDashboardPage() {
  ...
  try {
    const [venues, qrCodes, inventory, campaigns, payments, drafts, published] = await Promise.all([
      ...
      prisma.qrCode.findMany({ include: { venue: true, restroom: true, toiletLocations: true }, orderBy: { code: "asc" } }),
      ...
      prisma.adCampaign.findMany({ include: { advertiser: true, inventory: { include: { venue: true, restroom: true, qrCode: true } }, payments: true }, orderBy: { createdAt: "desc" } }),
    ...
    const pipelineRevenue = campaigns.filter((campaign) => ["PAYMENT_PENDING", "PAID", "APPROVED", "ACTIVE"].includes(campaign.status)).reduce((sum, campaign) => sum + campaign.priceCents, 0);
