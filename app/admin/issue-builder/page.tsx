...
export default async function IssueBuilderPage() {
  let issue: Awaited<ReturnType<typeof getDefaultIssue>> | null = null;
  ...
  return (
    <section>
      ...
      <p className="max-w-4xl font-bold">
        Drag-and-drop planning surface for monthly restroom issues. The MVP stores block order and layout JSON; use Edit Issue to persist each drop-zone assignment.
      </p>

      <JulyEventsImportButton />
          ...
          <div className="mt-6 grid gap-4 rounded-2xl border-4 border-ink bg-white p-5 shadow-brutal md:grid-cols-3">
            <div className="md:col-span-3">
              ...
              <p className="font-bold">
                Build next month in advance from the Schedule dashboard. Raw Schedule ISO is hidden; admins choose date, time, and timezone instead.
              </p>
            </div>

            <label className="grid gap-1 font-black uppercase">
              Issue Title
              <input defaultValue={issue.title} readOnly className="rounded border-2 border-ink p-3 font-bold normal-case" />
            </label>

            <label className="grid gap-1 font-black uppercase">
              Slug
              <input defaultValue={issue.slug} readOnly className="rounded border-2 border-ink p-3 font-bold normal-case" />
            </label>

            <label className="grid gap-1 font-black uppercase">
              Timezone
              <input defaultValue="America/Los_Angeles" readOnly className="rounded border-2 border-ink p-3 font-bold normal-case" />
            </label>

            <label className="grid gap-1 font-black uppercase">
              Publish Date
              <input type="date" className="rounded border-2 border-ink p-3 font-bold normal-case" />
            </label>

            <label className="grid gap-1 font-black uppercase">
              Publish Time
              <input type="time" className="rounded border-2 border-ink p-3 font-bold normal-case" />
            </label>
            ...
            <Link className="rounded bg-ink px-4 py-3 text-center font-black uppercase text-white md:col-span-3" href="/admin/schedule">
              Schedule Next Month
            </Link>
          </div>

          <div className="mt-4 flex gap-3">
            <Link className="rounded bg-ink px-4 py-3 font-black uppercase text-white" href="/admin/default-issue">
              Edit {issue.title}
            </Link>

            <Link className="rounded bg-stallYellow px-4 py-3 font-black uppercase" href="/issue">
              Preview
            </Link>
          </div>
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_320px]">
            <div className="grid gap-4 md:grid-cols-2">
              {issue.contentBlocks.map((block, index) => (
                <article key={block.id} draggable className="cursor-grab rounded-2xl border-4 border-dashed border-ink bg-white p-5 shadow-brutal">
                  <p className="text-xs font-black uppercase text-stallRed">
                    Drop zone {index + 1} • {contentLabels[block.type as keyof typeof contentLabels] || block.type}
                  </p>
                  <h2 className="font-display text-4xl uppercase">{block.title}</h2>
                  <p className="font-bold line-clamp-3">{block.body}</p>
                </article>
              ))}
            </div>
...
function PreviewSection({ label }: { label: string }) {
  return (
    <div className="rounded-2xl border-4 border-ink bg-white p-4">
      <p className="rounded-full bg-ink px-3 py-1 text-xs font-black uppercase tracking-widest text-stallYellow">
        Publication content
      </p>
      <h3 className="mt-2 font-display text-4xl uppercase">{label}</h3>
    </div>
  );
}
...
function PreviewAd({ slotNumber, name }: { slotNumber: number; name: string }) {
  return (
    <div className="rounded-2xl border-4 border-ink bg-white p-4 shadow-brutal">
      <p className="rounded-full bg-ink px-2 py-1 text-[10px] font-black uppercase tracking-widest text-stallYellow">
        {sponsorPlacementLabel(slotNumber)}
      </p>
      <div className={`ad-gradient-${slotNumber} mt-3 grid min-h-28 place-items-center rounded-xl border-2 border-ink px-4 text-center text-xl font-black uppercase text-white`}>
        {name}
      </div>
    </div>
  );
